...
- Sticky routing: tapping a shortcut the specialist just offered continues in that agent (skips the supervisor LLM). FAQ consultation / book-this-procedure offers set `lastHandoff.yieldToSupervisor` so «Так» is re-routed to booking. «Перенести» / «Скасувати» go to booking even after a FINISH visit list or an Already-booked replace offer
...
- Reply keyboards come from graph `lastHandoff.replyButtons` (one-time Telegram **reply keyboard**). Models write patient text only; accidental `<reply_buttons>` tags are stripped before send. Tapping a label sends that text as a normal message. Every keyboard ends with **«Головне меню»** (adapter-appended, including HITL and turns with no other shortcuts). Supervisor **FINISH** attaches DEFAULT MENU, or VISIT CHANGE («Перенести», «Скасувати», «Ні, дякую») after an explicit visit inquiry. `/start` and reminders still use `buildDefaultMenuKeyboard`. When `create_meeting` returns `Already booked`, booking finalize attaches REPLACE («Скасувати», «Ні, дякую») — never «Перенести». Consultation and book-this-procedure yes/no offers attach «Так» / «Обрати іншу процедуру» (FAQ also yields «Так» to booking). DATE/TIME shortcuts (dates plus «Інша дата», then HH:mm) come from the graph snapshot. Catalog drill-down shortcuts come from visible bullets. «Головне меню» routes to a short idle reply with DEFAULT MENU; during HITL it declines the write. English aliases (Book / Services / Address / …) are recognized for routing.
