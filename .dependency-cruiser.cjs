  ...
  forbidden: [
    ...
    },
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          '(^|/)(?:babel|webpack|vitest)[.]config[.](?:js|cjs|mjs|ts|cts|mts|json)$' // other configs
        ]
      },
      to: {},
    },
    {
      ...
      to: {
        ...
        path: [
          ...
          // async_hooks hosts AsyncLocalStorage (still supported); not flagged
          '^punycode$',
          '^domain$',
          '^constants$',
          '^sys$',
    ...
    },
    {
      ...
      to: {
        ...
        // consider type-only dependencyTypes for this rule. Same for npm-peer
        // when a package intentionally lists a dep as both dependency + peer.
    ...
    // Layer boundary rules (composition root may reach down; shared is a sink)
    {
      name: 'tools-not-to-composition',
      comment:
        'Tools must not depend on composition. Inject adapters via shared contracts (e.g. McpCallTool).',
      severity: 'error',
      from: { path: '^src/tools' },
      to: { path: '^src/composition' }
    },
    {
      name: 'config-not-to-graph',
      comment:
        'Config must not depend on graph. Keep env/bootstrap helpers in src/shared.',
      severity: 'error',
      from: { path: '^src/config[.]ts$' },
      to: { path: '^src/graph' }
    },
    {
      name: 'adapter-not-to-graph',
      comment:
        'Adapter must not depend on graph internals. Use shared helpers or composition facades.',
      severity: 'error',
    ...
    },
    {
      name: 'shared-is-leaf',
      comment:
        'shared/ must not import graph, composition, tools, or adapter (leaf layer).',
      severity: 'error',
      from: { path: '^src/shared' },
      to: {
        path: '^src/(graph|composition|tools|adapter)'
      }
    }
  ],
  options: {
...
// generated: dependency-cruiser@18.1.1 on 2026-08-07T09:27:29.499Z
